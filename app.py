    ...
    GROQ_KEY = st.sidebar.text_input("Groq API Key (or leave empty for local demo)", type="password", value=os.getenv("GROQ_API_KEY",""))
    ...
    SAMPLE_DOCS = [
        {"id":"doc1","title":"Product A Spec","content":"Product A is a compact device... It supports features X, Y, Z. Pricing starts at $199. Warranty 1 year."},
        {"id":"doc2","title":"Installation Guide","content":"To install Product A, first unbox. Then connect power. Follow safety guidelines..."},
        {"id":"doc3","title":"Troubleshooting","content":"Common issues include network, power, and firmware. For network reset, hold button 10s. For firmware, visit example.com/firmware."}
    ]
    documents = []
    if uploads:
        for f in uploads:
            text = f.read().decode('utf-8', errors='ignore')
            documents.append({"id":f"name_{int(time.time())}", "title":f.name, "content":text})
    if not documents:
    ...
    with tabs[0]:
        ...
        if st.button("Get Answer") and q:
            ...
            def llm_gen(q, context):
                ...
                try:
                    if GROQ_KEY:
                        ...
                        r = client.chat.completions.create(model="llama-3-oss-20b", messages=[{"role":"user","content":prompt}], temperature=0, max_tokens=400)
            ...
            answer = rag.generate_answer(q, chunks, llm_gen_fn=llm_gen if True else None)
    ...
    with tabs[1]:
        ...
        if st.button("Run eval"):
            ...
            st.json(results)
            st.download_button("Download results (json)", data=json.dumps(results, indent=2), file_name="eval_results.json")
    with tabs[2]:
        st.header("Documents")
        for d in documents:
    ...
    with tabs[3]:
        ...
        st.markdown("""**Chunking**: We chunk documents into ~150-word semantic chunks (sentences joined) to balance context size vs retrieval recall.

**Embedding store**: In-memory cached embeddings (suitable for small docsets). For larger corpora switch to Chroma/Pinecone/Weaviate.

**Answer gen**: Uses Groq OSS 20B when API key provided; otherwise falls back to extractive answers from retrieved chunks.
""")
